...
/**
 * TODO 可以对bean的属性进行替换，无论这个属性有没有默认值，都可以进行替换
 * 在应用程序上下文定义中重写bean属性值的属性资源配置程序。它将属性文件设置到bean definitions.
 ...
 * 与PropertyPlaceholderConfigurer相反，原始定义可以为此类bean属性具有默认值，也可以完全没有值。
 * 如果覆盖属性文件没有特定bean属性的条目，则使用缺省上下文定义。
